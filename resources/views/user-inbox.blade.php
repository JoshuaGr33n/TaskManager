...
<div class="main" id="app">
    ...
    <div class="todoListContainer">
        <div class="heading">
            <h2 id="title">Inbox{{$count_unread}}</h2>

        </div>

    </div>

    <a class="btn btn-sm btn-secondary" href="outbox">
        Outbox
    </a>
                ...
                <div class="modal-body">
                    ...
                    <form method="POST" action="{{url('send_message')}}" @submit.prevent="onSubmit">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="from" disabled value="{{Auth::user()->email}} (Me)">
                        </div>
                        <div class="form-group">
                            <label for="recipient-name" class="col-form-label">To:</label>
                            <input type="text" class="form-control" id="receipient" disabled value="Admin">
                        </div>
        ...
        <table id="datatable" class="table table-striped table-bordered nowrap" style="width:100%">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td id="title"> <span v-if="feedback === {{$message->id}}">
                            <span v-if="label === 'Read'" :class="['label label-success']">@{{label}}</span>
                    ...
                    @if($message->status == True)
                    <td id="title">
                        <span v-if="feedback === {{$message->id}}">
                            <a class="btn btn-sm btn-danger" @click="markUnRead({{$message->id}})">
                                Mark As Unread
                            </a>
                        </span>
                        <span v-else>
                            <a class="btn btn-sm btn-success" @click="markUnRead({{$message->id}})">
                                Mark As Unread
                            </a>
                        </span>
                    </td>
                    ...
                    <td id="title">
                        <span v-if="feedback === {{$message->id}}">
                            <a class="btn btn-sm btn-success" @click="markRead({{$message->id}})">
                                Mark As Read
                            </a>
                        </span>
                        <span v-else>
                            <a class="btn btn-sm btn-danger" @click="markRead({{$message->id}})">
                                Mark As Read
                            </a>
                        </span>
                    ...
                    @endif
                    <td id="title"><a class="btn btn-xs btn-primary" href="inbox/{{$message->id}}">
...
<script type="text/javascript">
    const app = new Vue({
        ...
        methods: {
            ...
            markRead(id) {
                console.log(id);
                axios.put('{{url("update_message_status")}}/' + id, {
